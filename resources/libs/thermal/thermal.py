...
class ThermalPrinter(Serial):
    ...
    def __init__(self, port, baudrate=9600,*args,**kwargs):
        #os.system("/home/pi/pilaroid/resources/libs/thermal/rpirtscts.sh")
        Serial.__init__(self, port, baudrate,*args,**kwargs)
        #self.wake()       
        self.dots = 5
        self.heattime = 180
        self.heatinterval = 200
        self.density = 12
        self.breaktime = 7
        self.reset()


    def setPrintSettings(self, heatdot=None, heattime=None, heatinterval=None): #heatime=120
        heatdot = heatdot or self.dots
        heattime = heattime or self.heattime
        heatinterval = heatinterval or self.heatinterval
    ...
    def setPrintDensity(self, density=None, breaktime=None):
        density = density or self.density
        breaktime = breaktime or self.breaktime
        self.writeBytes(
            18,  # DC2
            35,  # Print density
            (breaktime << 5) | density)
    ...
    def hasPaper(self):
        ...
        for i in range(8) :   
            ...
            try :
                ...
                pass
        #stat = ord(self.read(8)) #& 0b00000100
...
if __name__ == "__main__" :
    printer = ThermalPrinter("/dev/serial0", 9600, timeout=1)
    printer.setCodePage(0)
    
    printer.println('testé test test')
    printer.setCodePage(4)
    printer.setCharset(1)
    printer.println('testé test test')
    #printer.testPage()
